...
The RTOS provides several mechanisms to move interrupt processing onto a thread. These include, but are not limited to:
 ...
 - [Queue](https://os.mbed.com/docs/v5.10/mbed-os-api-doxy/classrtos_1_1_queue.html).
 - [Mail](https://os.mbed.com/docs/v5.10/mbed-os-api-doxy/classrtos_1_1_mail.html).
...
For more information, please see [rtos/Mutex.h](https://os.mbed.com/docs/v5.10/mbed-os-api-doxy/_mutex_8h_source.html).
...
For more information see [platform/critical.h](https://os.mbed.com/docs/v5.10/mbed-os-api-doxy/critical_8h_source.html).
...
- Nanostack - **Not protected**:  in general, we recommend that Mbed developers use our 6LoWPAN stack through the Network Socket API. If you wish to use Nanostack directly, you need to be aware of how it uses threads. The core of Nanostack runs on a tasklet mechanism, scheduled on a single underlying OS thread. Developers who wish to call directly to Nanostack must create their own tasklet and make Nanostack calls from there. Because there is only one OS thread servicing all tasklets, there is no need for further synchronization between tasklets. See the [6LoWPAN documentation](/docs/v5.10/apis/mesh-api.html).
- `mbed-tls` - **Not protected**: function calls are safe from any thread as long as the objects they operate on are properly protected - see the [documentation](https://tls.mbed.org/kb/v5.10/thread-safety-and-multi-threading).
