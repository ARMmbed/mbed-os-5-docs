...
LittleFS provides several configuration options that can be used to tweak the performance of the file system on different hardware. By default, LittleFS will try to find the optimal configuration from the underlying block device's geometry, but this can be overriden to optimize special situations. For example, if your device has a large amount of RAM, the `read_size` and `prog_size` configuration options can be increased for a minor speed improvement.

Note that LittleFS has 4 levels of debug logging. By default, all logging is enabled except for `enable_debug`. Setting `enable_debug` to `true` will make the log output very verbose, and the output is very useful for bug reports.
...
NVStore does not need much configuration, relying only on the regions of internal flash specified in the `area_*_address` and `area_*_size` for the two areas. Additionally, `max_keys` can be used to manage the amount of RAM needed for NVStore keys. Note that `max_keys` will default to the number of keys needed by Mbed OS and only needs to be modified if an application uses NVStore directly.
