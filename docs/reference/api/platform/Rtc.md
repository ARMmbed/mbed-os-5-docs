...
You can use the `attach_rtc` API to hook external RTC for using C time functions. It provides you with `init()`, `read()`, `write()` and `isenabled()` functions to be attached. [Time](/docs/v5.7/reference/time.html) provides more information about C `date` and `time` standard library functions.
...
[![View code](https://www.mbed.com/embed/?type=library)](http://os.mbed.com/docs/v5.7/mbed-os-api-doxy/mbed__rtc__time_8h_source.html)
