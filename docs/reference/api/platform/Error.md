...
- File name and line number - By default, capturing of file name and line number is disabled. If you need this information to be captured, you need to enable it by using the **MBED_CONF_PLATFORM_ERROR_FILENAME_CAPTURE_ENABLED** macro.
...
Below is an example of terminal output the `MBED_ERROR1()` call created. Note that file name capture is disabled by default, even for debug and develop builds. You can enable the filename capture by defining `MBED_CONF_PLATFORM_ERROR_FILENAME_CAPTURE_ENABLED`.
