...
The RTOS provides several mechanisms to move interrupt processing onto a thread. These include, but are not limited to:
 ...
 - [Queue](https://os-doc-builder.test.mbed.com/docs/v5.8/mbed-os-api-doxy/Queue_8h_source.html).
 - [Mail](https://os-doc-builder.test.mbed.com/docs/v5.8/mbed-os-api-doxy/Mail_8h_source.html).
...
For more information see [rtos/Mutex.h](https://os-doc-builder.test.mbed.com/docs/v5.8/mbed-os-api-doxy/Mutex_8h_source.html).
...
- Nanostack - **Not protected**:  in general, we recommend that Mbed developers use our 6LoWPAN stack through the Network Socket API. If you wish to use Nanostack directly, you need to be aware of how it uses threads. The core of Nanostack runs on a tasklet mechanism, scheduled on a single underlying OS thread. Developers who wish to call directly to Nanostack must create their own tasklet and make Nanostack calls from there. Because there is only one OS thread servicing all tasklets, there is no need for further synchronization between tasklets. See the [6LoWPAN documentation](/docs/v5.8/tutorials/mesh.html).
