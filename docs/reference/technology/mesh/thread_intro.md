...
Mbed Thread is implemented in the Nanostack library, which also supports the 6LoWPAN protocol. In Mbed OS, the Thread stack runs in its own RTOS thread using an internal event scheduler. Mbed OS provides the [Mesh C++ API](/docs/v5.9/reference/mesh-api.html) for building Thread applications.
...
See [Thread commissioning guide](/docs/v5.9/reference/technology.html#thread-commissioning) how to commission a Thread device to the network in practice.
