...
Mbed Thread is implemented in the Nanostack library, which also supports the 6LoWPAN protocol. In Mbed OS, the Thread stack runs in its own RTOS thread using an internal event scheduler. Mbed OS provides the [Mesh C++ API](/docs/v5.10/apis/mesh-api.html) for building Thread applications.
...
- For the socket communication over the Thread network, use the [Mbed sockets API](/docs/v5.10/apis/network-socket.html).
