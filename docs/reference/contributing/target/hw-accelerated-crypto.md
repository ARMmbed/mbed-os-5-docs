...
Mbed TLS has separate modules for the different cryptographic primitives, each containing multiple function building blocks. Hardware acceleration interfaces are available both on the module and the function level. In the following list, every module listed can be replaced as a whole, and any function listed can alternatively be replaced on its own:
