...
[Mbed CLI](/docs/v5.9/tools/arm-mbed-cli.html) currently supports [exporting](/docs/v5.9/tools/exporting.html#exporting-from-arm-mbed-cli) to all of the development environments mentioned above by using the `export` command.
...
When you export from Mbed CLI, you create a project that compiles with the debug profile. You can find more information on the debug profile in the [build profiles documentation](/docs/v5.9/tools/build-profiles.html). For example, this means that compiling within UVision 5 after this export:
