...
For repeating asynchronous cases, you can "add" both attributes together: `CaseTimeout(200) + CaseRepeatAll` will wait for 200 ms for the callback validation and then repeat the test case. See the section on <a href="/docs/v5.7/tools/utest-asynchronous-c-test-harness.html#test-case-attribute-arbitration" target="_blank">arbitration logic</a> for more details.
