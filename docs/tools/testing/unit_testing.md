...
This document describes how to write and test unit tests for Mbed OS. If you run into any problems see [troubleshooting](#troubleshooting) section.

### Introduction

Unit tests test code in small sections on host machine. Other testing tools for Mbed OS require embedded hardware and full Mbed OS to be built, which result to slower tests. Unit testing takes place in a build environment where we test each C or C++ class or module in isolation. This means we build test suites into separate test binaries and stub all access outside to remove dependencies on any specific embedded hardware or software combination. This allows us to complete the testing using native compilers on the build machine.

### Prerequisites

Please install all the following dependencies to use Mbed unit testing. See detailed instructions for supported operating systems below.
...
```
3. Install Gcovr and [Mbed CLI](https://os.mbed.com/docs/latest/tools/arm-mbed-cli.html) with `pip install "gcovr>=4.1" mbed-cli`.
...
5. Install Gcovr and [Mbed CLI](https://os.mbed.com/docs/latest/tools/arm-mbed-cli.html) with `pip install "gcovr>=4.1" mbed-cli`.

### Test code structure

Unit tests are located in Mbed OS repository under the `UNITTESTS` folder. Unit test files should use identical directory path to the file under test. This makes it easier to find unit tests for a particular class or a module. For example, if the file under test is `some/example/path/ClassName.cpp`, then all the test files should be in the `UNITTESTS/some/example/path/ClassName` directory. Each test suite needs to have their own `unittest.cmake` file for test configuration.
...
Names of test suites are automatically generated by the build system. The name is constructed by taking a relative file path from the UNITTESTS directory to the test directory and replacing path separators with dashes. For example test suite name for `some/example/path/ClassName.cpp` is `some-example-path-ClassName`. Suite names are used when deciding which test suites to run.

### Unit testing with Mbed CLI

Mbed CLI supports unit tests through `mbed test --unittests` command. To learn how to use unit tests with Mbed CLI, please see the [appropriate handbook section](https://os.mbed.com/docs/latest/tools/test-and-debug.html#unit-testing). For other information on using Mbed CLI, please see the [CLI documentation in handbook](https://os.mbed.com/docs/latest/tools/arm-mbed-cli.html).
...
List all the files required for the build in the `unittest.cmake` file. The file paths listed should be relative to `UNITTESTS` folder. Use the following variables to list the source files and include paths:

* **unittest-includes** - List of header include paths. Can be used to extend or overwrite default paths listed in CMakeLists.txt
* **unittest-sources** - List of files under test.
* **unittest-test-sources** - List of test sources and stubs.

Here are listed steps to write a simple unit test. In this example we take `rtos/Semaphore.cpp` as a class under test.
...
3. Run CMake using relative path to `UNITTESTS` folder as argument. So from `UNITTESTS/build` use `cmake ..`:
    ...
    * See [CMake manual](https://cmake.org/cmake/help/v3.0/manual/cmake.1.html) for more information.
4. Run a make program (make, gmake, mingw32-make, etc) to build the tests.

#### Run tests directly with CTest

Run a test binary in the build directory to run a unit test suite. To run multiple test suites at once use CTest test runner. Run CTest with `ctest`. Add `-v` to get results for each test case. See [CTest manual](https://cmake.org/cmake/help/v3.0/manual/ctest.1.html) for more information.
