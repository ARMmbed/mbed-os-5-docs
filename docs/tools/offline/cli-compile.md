...
- `-t <TOOLCHAIN>` selects a toolchain (of those defined in `mbed_settings.py`, see above). The value can be `ARM` (Arm Compiler), `GCC_ARM` (GNU Arm Embedded) or `IAR` (IAR Embedded Workbench for Arm).
   <span class="notes">**Note**: `mbed compile -t ARM` selects the Arm Compiler major version based on the Arm archetecture version of your target. Arm archetectures version 6 and 7 use Arm Compiler 5, and Arm archetecture version 8 uses Arm Compiler 6 </span>
