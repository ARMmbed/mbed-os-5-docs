...
<span class="notes">**Note**: Mbed CLI stores information about libraries and dependencies in reference files that use the `.lib` extension (such as `lib_name.lib`). Although these files are human-readable, we *strongly* advise that you don't edit these manually - let Mbed CLI manage them instead.</span>
...
For more information on build profiles, see [our build profiles](/docs/v5.8/tools/build-profiles.html) and [toolchain profiles](/docs/v5.8/tools/toolchain-profiles.html) pages.
...
The [compile configuration system](/docs/v5.8/tools/configuring-tools.html) provides a flexible mechanism for configuring the Mbed program, its libraries and the build target.
