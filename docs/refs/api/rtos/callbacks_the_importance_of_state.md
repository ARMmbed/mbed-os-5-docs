...
A callback is a user provided piece of code that is passed to another function. The callback allows an API to execute the user’s code in its own context. More info on how to use callbacks can be found in the [technical callback documentation](callbacks.md)
