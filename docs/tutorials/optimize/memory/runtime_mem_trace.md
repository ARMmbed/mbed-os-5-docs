...
The memory tracer is not enabled by default. To enable it, you need to enable the **`memory-tracing-enabled`** setting in the Mbed OS Platform config options. The recommended way to enable this setting is to add it to your `mbed_app.json`:
