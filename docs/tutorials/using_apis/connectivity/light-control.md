...
The application below demonstrates a simple light control application, where devices can control the LED status of all devices in the network. You can build the application for the unsecure 6LoWPAN-ND network.
...
[![View code](https://www.mbed.com/embed/?url=https://github.com/ARMmbed/mbed-os-example-mesh-minimal)](https://github.com/ARMmbed/mbed-os-example-mesh-minimal/blob/mbed-os-5.13/mesh_led_control_example.cpp)
...
The following tables show the values to use in the `mbed_app.json` file for your devices in different networks. For a 6LoWPAN-ND based network, use `nsapi.default-mesh-type: LOWPAN`.
...
Nanostack uses internal heap, which you can configure in `mbed_app.json`:
