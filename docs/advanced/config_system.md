...
- `target_overrides` is a dictionary with target-specific values for the configuration parameters. It is used to override the default values of these parameters so that they match the current compilation target. 
    - The keys in `target_overrides` are matched against toolchain *labels* (for more information see "mbed Targets" - coming soon). If a key inside `target_overrides` matches one of the target labels, the parameter values receive the key values. 
